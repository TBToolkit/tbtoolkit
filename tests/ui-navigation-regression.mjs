...
assert.match(css,/Fit the complete encounter[\s\S]*battle-encounter-fields\{grid-template-columns:205px 196px\}[\s\S]*battle-norm-input\{grid-template-columns:minmax\(0,60px\) 50px 74px/,'The encounter and all three norm controls must fit inside the fixed Battle card.');
...
assert.match(css,/battle-norm-input\{grid-template-columns:minmax\(0,60px\) 50px 74px/,'The K/M/B selector must reserve enough width for its visible unit text.');
