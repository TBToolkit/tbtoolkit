...
assert.match(css,/Fit the complete encounter[\s\S]*battle-encounter-fields\{grid-template-columns:180px 222px\}[\s\S]*battle-norm-input\{grid-template-columns:minmax\(0,72px\) 56px 82px/,'The encounter and all three norm controls must fit inside the fixed Battle card.');
...
assert.match(css,/#battleBetaPanel\{height:100%;grid-template-rows:auto minmax\(0,1fr\);align-content:stretch;gap:16px\}/,'Player and Battle cards must remain separated while Battle aligns with the adjacent card bottoms.');
