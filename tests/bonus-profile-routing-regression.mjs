...
for(const species of ['BEAST','DRAGON','ELEMENTAL','GIANT'])assert.equal(bonusProfileForUnit({species,unitClass:'SPECIALIST'}),species);
...
const bonuses=deriveBonusInputs({
  ...
  customProfileBonuses:{
    beastHealthPct:900,beastStrengthPct:1800,beastDDPct:9,beastSTPct:18,
...
assert.equal(beast.bonusFamily,'BEAST');
assert.equal(beast.effectiveHealth,100);
assert.ok(Math.abs(beast.pDD-.14)<1e-12);
assert.equal(cursed.bonusFamily,'GUARDSMAN');
assert.equal(cursed.effectiveHealth,20);
assert.ok(Math.abs(cursed.pDD-.06)<1e-12);

console.log(JSON.stringify({ok:true,routes:11}));
