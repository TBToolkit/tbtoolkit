...
assert.match(html,/>Player Account<select id="netPlayerAccount"/,'Clan Overview must identify the account supplying net battle results.');
assert.match(html,/<select id="tinmanRevivalStrategy"[^>]*><option value="full">Revive All<\/option><option value="mercenary-monster">Revive Mercs \+ Monsters<\/option><option value="mercenary-only">Revive Mercs Only<\/option><\/select>/,'Tinman must show the renamed revival strategies without changing their saved values.');
assert.match(script,/strategyLabel=value=>REVIVAL_STRATEGIES\[value\]/,'Clan Overview summaries must use the same revival labels as its selectors.');
