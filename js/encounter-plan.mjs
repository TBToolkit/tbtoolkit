...
export function calculateEncounterPlan({normPoints,pointsPerAttack,goldByCategory={},rebuildRows=[],strategy='full'}={}){
  ...
  let silverPerHit=0,dragonCoinsPerHit=0,mercenariesConsumedPerHit=0,rebuildCostsComplete=true;
  for(const row of rebuildRows){
    ...
    if(category==='mercenary'){
      mercenariesConsumedPerHit+=rebuildQuantity;
      continue;
    }
  ...
  return{
    hits,goldPerHit,silverPerHit,dragonCoinsPerHit,mercenariesConsumedPerHit,rebuildCostsComplete,
    totalGold:hits*goldPerHit,totalSilver:hits*silverPerHit,totalDragonCoins:hits*dragonCoinsPerHit,
    totalMercenariesConsumed:hits*mercenariesConsumedPerHit,
