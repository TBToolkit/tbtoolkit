/* TB Toolkit Epic Optimizer 2.0 Paired Counterfactual v190-dev1
 *
...
async function loadSharedCombat(){
  if(!sharedCombatPromise){
    const url = new URL('./epic-combat-engine-v2.mjs?v=190-dev1', self.location.href);
    sharedCombatPromise = import(url.href).then((mod)=>{
      ...
      if(typeof deriveBonusInputs!=='function'||typeof buildSquad!=='function'||typeof scoreEpicArmy!=='function'||typeof validateArmyDatabase!=='function')
        throw new Error('Shared Epic combat module is missing required exports.');
      return mod;
    });
  }
...
function optimizeFromSeed({
  ...
  if (!candidateFeasible({ result, limits, minimumHealthSeparationPct }) || (structureValidator && !structureValidator(result, selected))) {
      ...
      for (const group of groups) {
        ...
        for (const receiver of group.units) {
          for (const donor of group.units) {
            ...
            evaluations += 1;
            if (!candidateFeasible({ result:candResult, limits, minimumHealthSeparationPct }) || (structureValidator && !structureValidator(candResult, selected))) continue;
        ...
        for (const u of group.units) {
          ...
          if (current - delta >= minimumQuantity) {
            ...
            const candResult = scoreEpicArmy({ units, quantities:cand, bonuses });
            evaluations += 1;
            if (candidateFeasible({ result:candResult, limits, minimumHealthSeparationPct }) && (!structureValidator || structureValidator(candResult, selected)) && compareScore(candResult.expectedTotalLifetimeDamage, bestCandidateScore)) {
...
self.onmessage=async(event)=>{
 ...
 try{
  ...
  self.postMessage({type:'result',requestId,payload:result,diagnostics:{
   optimizerBuild:EPIC_OPTIMIZER_BUILD,engineBuild:EPIC_COMBAT_ENGINE_BUILD,mechanicsBuild:EPIC_MECHANICS_BUILD,armyDatabase:'ARMY9-v72',
