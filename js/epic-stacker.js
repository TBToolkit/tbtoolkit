...
function wireEvents(){document.querySelectorAll('.mode-button').forEach(b=>b.addEventListener('click',()=>switchMode(b.dataset.mode)));els.clearAllSelections.addEventListener('click',clearAllSelections);els.resetCalculator.addEventListener('click',resetCalculator);const inputGroups=[['leadership','authority','dominance'],['monsterHealth','humanHealth','epicHunterHealth']];for(const group of inputGroups){for(const id of group){const input=els[id];input.addEventListener('focus',()=>{input.value=String(parseNumber(input.value)||'');requestAnimationFrame(()=>input.select());});input.addEventListener('pointerup',e=>{e.preventDefault();input.select();});input.addEventListener('keydown',e=>{if(e.key!=='Tab')return;const index=group.indexOf(id),nextIndex=index+(e.shiftKey?-1:1);if(nextIndex<0||nextIndex>=group.length)return;e.preventDefault();const next=els[group[nextIndex]];next.focus();requestAnimationFrame(()=>next.select());});input.addEventListener('blur',()=>formatFieldInteger(input));input.addEventListener('input',recalculate);}}for(const id of ['leadershipFill','authorityFill','dominanceFill']){
