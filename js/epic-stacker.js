...
function setView(view){
  const unitView=['troop','monster','mercenary'].includes(view);
  if(unitView)activeCategory=view;
  document.querySelectorAll('.category-tab[data-view]').forEach(b=>{
    const active=b.dataset.view===view;
    b.classList.toggle('active',active);
    b.setAttribute('aria-selected',String(active));
  });
  els.selectionContent.hidden=!unitView;
  els.resultsView.hidden=view!=='results';
  els.layerChartPanel.hidden=view!=='visual';
  if(unitView)renderSelectionGrid();
}
...
function wireEvents(){document.querySelectorAll('.category-tab[data-view]').forEach(b=>b.addEventListener('click',()=>setView(b.dataset.view)));els.clearCategory.addEventListener('click',clearCurrent);els.resetCalculator.addEventListener('click',resetCalculator);for(const id of ['leadership','authority','dominance','monsterHealth','humanHealth','epicHunterHealth']){els[id].addEventListener('focus',()=>{els[id].value=String(parseNumber(els[id].value)||'');});els[id].addEventListener('blur',()=>formatFieldInteger(els[id]));els[id].addEventListener('input',recalculate);}for(const id of ['leadershipFill','authorityFill','dominanceFill'])els[id].addEventListener('input',recalculate);els.rankSeparation.addEventListener('input',()=>{updateRankSeparationDisplay();recalculate();});els.resetRankSeparation.addEventListener('click',()=>{els.rankSeparation.value='0.40';state.inputs.rankSeparation='0.40';updateRankSeparationDisplay();saveState();recalculate();});for(const id of ['autoLeadership','autoAuthority','autoDominance'])els[id].addEventListener('change',()=>{readInputs();updateFillFieldStates();recalculate();});els.arachne.addEventListener('change',recalculate);}
async function init(){cacheElements();loadSavedState();applyStateToInputs();wireEvents();try{await loadData();migrateLegacySelections();updateCounts();setView('troop');recalculate();}catch(error){console.error(error);showValidation(['The unit database could not be loaded. Refresh the page and try again.']);}}
init();
